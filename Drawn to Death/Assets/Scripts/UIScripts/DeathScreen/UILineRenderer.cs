...
// Class taken from https://github.com/Radishmouse22/UILineRenderer/blob/main/UILineRenderer.cs
...
namespace Radishmouse
{
    ...
    public class UILineRenderer : MaskableGraphic
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            // This part adds a connection from the last point to the first point
            int startIndex = vh.currentVertCount;
            int last = points.Length - 1;
            CreateLineSegment(points[last], points[0], vh);
            int index2 = startIndex;

            vh.AddTriangle(index2, index2 + 1, index2 + 3);
            vh.AddTriangle(index2 + 3, index2 + 2, index2);

            // Bevel join between last normal segment and loop segment
            vh.AddTriangle(index2, index2 - 1, index2 - 3);
            vh.AddTriangle(0, 1 , index2+2); // Not the correct edge but it is visually okay if there is no line opacity
